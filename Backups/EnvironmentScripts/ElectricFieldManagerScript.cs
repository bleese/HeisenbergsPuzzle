...
public class ElectricFieldManagerScript : MonoBehaviour {
	public GameObject efield; // Electric Field prefab;
	private GameObject tempField; 
	GameObject selectedField = null;
	ElectricFieldScript selectedScript = null;
	List<GameObject> eFields = new List<GameObject>();
	...
	public void createField() {
		...
		tempField = Instantiate (efield,fieldPosition, Quaternion.identity) as GameObject;
		eFields.Add (tempField);		
	}
	...
	public void SetEditor(bool edit) {
		ElectricFieldScript tempScript = null;
		for (int counter = 0; counter < eFields.Count; counter++) {
			tempScript = eFields[counter].GetComponent<ElectricFieldScript>();
	...
	public void select(GameObject field) {
		selectedField = field;
		selectedScript = field.GetComponent<ElectricFieldScript>();
	}
